...
from .cli import UserOptions, ConfigFiletype
...
def print_state(state: OdbVisualizer, user_options: UserOptions) -> None:
    lines: list[dict[str, str]] = [
        {
            ".hdf5 file": f"{state.hdf_file if hasattr(state, 'hdf_file') else 'not set'}",
            ".odb file": f"{state.odb_file if hasattr(state, 'odb_file') else 'not set'}",
        },
        {
...
def process_input(state: OdbVisualizer) -> Union[UserOptions, Any]: # Returns UserOptions or Pandas Dataframe
    """
    ...
    plot_parser.add_argument("-l", "--low-temp", default=300, help="Temperature lower bound, defaults to 300 K")
    plot_parser.add_argument("-S", "--seed-size" help="Mesh seed size of the .odb file")
    plot_parser.add_argument("-t", "--time-sample", default=1, help="Time-sample value (N for every Nth frame you extracted). Defaults to 1")
    ...
    plot_parser.add_argument("--low-time", default=0, help="Lower time limit, defaults to 0 (minimum possible)")
    plot_parser.add_argument("--high-time", default=float("inf"), help="Upper time limit, defaults to infinity (max possible)")

    # TODO Default
    #plot_parser.add_argument("-v", "--view")
    ...
    # TODO
    # We need to check against the default config file with platformdirs
    # Then we overwrite those with whatever's in the input file
    # Then we overwrite those with the CLI switches
    # So, here, we need to set them all once, update them from those 3 sources, and then set them up to go.
    if args.config_file:
